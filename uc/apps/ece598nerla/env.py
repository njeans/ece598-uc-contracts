...
def env_committer_crupt_bad_c(k, static, z2p, _, z2a, a2z, f2z, p2z, pump):
    ...
    rand_x_val={}
    rand_str_val={}
    rand_x_val["12"]=Fq(64507471261611761880264178918529909889494982960490464220285243851186690568124)
    rand_str_val["12"]=b'E\x18 \xfa\xe0\xb9\xfe\xd9\xa2\x89\x88\xc9\xdfE\xec\xa4\xa4\xbc\xc5\xea\xa8s(\n\xe4\xda\xa19)\xbe\x11x'

    rand_x_val["13"]=Fq(59987769212432957123111507484962972584642453038669683369985153539480782463362)
    rand_str_val["13"]=b'+\xeaO\xfc\xaeu\x99\xb2\xfdD\xb9%-\xa1[\xaa!\xdb\xb3_\xbbp\x81_\x10<\xd2\x9c\xc5P\xe5\xb9'
    ...
    def _run(sender, receiver):
        rand_x =rand_x_val[str(sender)+str(receiver)]
        _m = solve(rand_x)
        _sid = Ginv(_m)[:2]
        rand_str =rand_str_val[str(sender)+str(receiver)]
        ...
        z2a.write( ('A2P', (sender, ('sendmsg', receiver, ('commit',(_cid, _sid,_c))))))
        waits(pump)

        z2a.write( ('A2P', (sender, ('sendmsg', receiver, ('reveal',(_cid, "fake"))))))
        _,res = waits(a2z)
        cprime=res[1][2][1][1]
        ...
        z2a.write( ('A2P', (sender, ('sendmsg', receiver, ('b',step_b_msg)))))
        _,res = waits(a2z)
        e=gevent.spawn(_a2z)
        ...
        z2a.write( ('A2P', (sender, ('sendmsg', receiver, ('d',(_cid,_z))))))
...
def env_committer_crupt_bad_a(k, static, z2p, z2f, z2a, a2z, f2a, p2z, pump):
    ...
    sid = ('one', "1, 2, 3, 4")
    static.write( (('sid',sid), ('crupt', 1, 4)))
...
def env_receiver_crupt(k, static, z2p, z2f, z2a, a2z, f2z, p2z, pump):
    ...
    def _run(sender,receiver):
        ...
        res=waits(a2z)
        z2p.write( (sender, ('reveal',receiver,cid)))
        res=waits(a2z)
        ...
        z2a.write( ('A2P', (receiver, ('sendmsg', sender, ('a',(cid,cprime))))))
        res=waits(a2z)
        ...
        z2a.write( ('A2P', (receiver, ('sendmsg', sender, ('c',(cid,(Rhat,Shat,epsilonhat)))))))
        e=gevent.spawn(_a2z)
        waits(pump)
        e.kill()
    _run(1,2)
